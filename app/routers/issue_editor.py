...
def make_issue_list_keyboard(issues: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    ...
    # Для каждой проблемной позиции - отдельная кнопка
    for i, issue in enumerate(issues):
        # Готовим текст кнопки (обрезаем, если слишком длинный)
        item_name = issue.get("invoice_item", "").split(' ')[0]  # Берем только название без единиц
        item_name = item_name[:15] + "..." if len(item_name) > 15 else item_name
        
        issue_type = issue.get("issue", "Проблема")
        # Эмодзи в зависимости от типа проблемы
        emoji = "❌" if "Not in database" in issue_type else "⚠️"
